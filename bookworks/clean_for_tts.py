...
import os
import argparse
from bookworks.bw_utils import clean_markdown_for_tts, split_by_chapters, epub_to_markdown  # Added import for epub_to_markdown
...
def process_file(input_file: str, book_title: Optional[str] = None, split_chapters: bool = False, max_chars: int = 3000):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--book-title", help="Optional book title (default taken from filename)")
    parser.add_argument("--split-chapters", action="store_true", help="Split output by chapters")
    parser.add_argument("--max-chars", type=int, default=3000, help="Maximum characters per output file")
    ...
    output_paths = process_file(args.input_file, args.book_title, args.split_chapters, args.max_chars)
    if len(output_paths) == 1:
        print(f"Successfully created TTS-ready markdown: {output_paths[0]}")
    else:
        print(f"Successfully created {len(output_paths)} TTS-ready markdown files in {OUTPUT_DIR}") 